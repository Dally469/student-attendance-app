...
class _AttendancePageState extends State<AttendancePage>
  ...
  // Store selected attendance ID for Clock In/Clock Out
  ...
  void initState() {
    ...
    // Load current checked-in students if attendanceId is provided
    if (widget.attendanceId != null && widget.attendanceId!.isNotEmpty) {
      ...
      // Start NFC immediately if we have attendance ID
      checkNfcAvailability();
    }

    // Check for multiple attendances and show selection dialog if needed
  ...
  Future<void> _checkAndSelectAttendance() async {
    ...
    try {
      ...
      final attendances =
          await _attendanceService.getAllAttendancesByClassroomDate(
        widget.classroomId!,
        dateStr,
      );

      // Auto-select attendance: prefer CHECK_IN_OUT if available, otherwise use first one
      if (attendances.isNotEmpty) {
        // Prefer CHECK_IN_OUT mode if available, otherwise use the first attendance
        final checkInOut =
            attendances.where((a) => a.mode == 'CHECK_IN_OUT').toList();
        if (checkInOut.isNotEmpty) {
          _selectedAttendanceId = checkInOut.first.id;
        } else {
          _selectedAttendanceId = attendances.first.id;
        }

        if (_selectedAttendanceId != null &&
            _selectedAttendanceId!.isNotEmpty) {
          _attendanceController
              .loadCurrentAttendanceStudents(_selectedAttendanceId!);
        }
      }
    } catch (e) {
      print('Error checking attendances: $e');
    }
  }
    ...
    ever(_attendanceController.successMessage, (message) {
      if (message.isNotEmpty) {
        ...
        Future.delayed(const Duration(milliseconds: 300), () {
          if (mounted) {
            final title = message.toLowerCase().contains('check-out')
                ? 'Checked Out'
                : 'Success';
            Get.snackbar(
              title,
              message,
              backgroundColor: message.toLowerCase().contains('check-out')
                  ? Colors.orange
                  : Colors.green,
    ...
    if (widget.attendanceId == null || widget.attendanceId!.isEmpty) {
      ...
      if (attendanceId.isNotEmpty) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          Get.offAndToNamed(makeAttendance, parameters: {
        ...
        onDiscovered: (NfcTag tag) async {
          try {
            ...
            if (tagId != null) {
              ...
              print('Tag ID is null - card format not recognized');
              // Restart session to continue scanning
              await NfcManager.instance.stopSession();
              await Future.delayed(const Duration(milliseconds: 1000));
              if (continuousScanning && mounted) {
                startNfcSession();
            ...
            try {
              ...
              print('Error stopping NFC session: $stopError');
            }
            ...
            await Future.delayed(const Duration(milliseconds: 1500));
            if (continuousScanning && mounted) {
              print('Restarting NFC session after error...');
              startNfcSession();
            }
          }
        },
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                      ...
                      child: Row(
                        ...
                        children: [
                          _buildStatColumn(
                            ...
                            value: _classroomStudentController.students.length
                                .toString(),
                            label: 'Total',
                            color: theme.colorScheme.primary,
                          ),
                          ...
                          Obx(() => _buildStatColumn(
                                ...
                                value: _attendanceController.checkInCount.value
                                    .toString(),
                                label: 'Present',
                                color: Colors.green,
                              )),
                          ...
                          Obx(() => _buildStatColumn(
                                ...
                                value: ((_classroomStudentController
                                            .students.length) -
                                        _attendanceController
                                            .checkInCount.value)
                                    .toString(),
                ...
                Obx(() {
                  final lastName =
                      _attendanceController.lastCheckedInStudentName.value;
                  if (lastName.isEmpty) {
